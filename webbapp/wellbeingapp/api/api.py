...
def get_current_time():
    ...
    float_list =  list(result[0])
    round_list = []
    for i in float_list :
        round_list.append(int(round(i))) 

    return {"result" : round_list , "BMU" : [int(BMU[0]) ,int(BMU[1])] } # jsonify the data and send it back in json format to the client (React application)
